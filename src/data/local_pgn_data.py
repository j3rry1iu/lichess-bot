import io
import chess.pgn
import torch
from torch.utils.data import Dataset

from board_encoder import encode_board
from utils.move_index import move_to_index
from data.lichess_stream_dataset import result_to_value  # reuse helper


class LocalPGNDataset(Dataset):
    """
    Dataset built from a single PGN file in your repo.
    Perfect for local debugging of the full pipeline.
    """

    def __init__(self, pgn_path: str, max_moves: int | None = None):
        self.samples = []

        with open(pgn_path, "r") as f:
            pgn_text = f.read()

        game = chess.pgn.read_game(io.StringIO(pgn_text))
        if game is None:
            raise ValueError(f"Could not parse PGN at {pgn_path}")

        value_label = result_to_value(game.headers.get("Result", "*"))

        board = game.board()
        moves = list(game.mainline_moves())
        if max_moves is not None:
            moves = moves[:max_moves]

        for move in moves:
            encoded = encode_board(board)
            policy_idx = move_to_index(move)

            self.samples.append((
                torch.tensor(encoded, dtype=torch.float32),
                torch.tensor(policy_idx, dtype=torch.long),
                torch.tensor(value_label, dtype=torch.float32),
            ))

            board.push(move)

        print(f"[LocalPGNDataset] Loaded {len(self.samples)} positions from {pgn_path}")

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        return self.samples[idx]
